...
def build_results_dir(tf_iter, newton_iter, optimizer_name):
    ...
    results_dir = os.path.join(PROJECT_ROOT, "Results", "Kuramoto-Sivashinsky", optimizer_folder, run_folder)
...
def loss(x_f_batch, t_f_batch,
    ...
    u_lb_pred, u_x_lb_pred, u_xx_lb_pred = u_x_model(u_model, x_lb, t_lb)
    u_ub_pred, u_x_ub_pred, u_xx_ub_pred = u_x_model(u_model, x_ub, t_ub)
    ...
    mse_b_u = tf.reduce_mean(tf.square(tf.math.subtract(u_lb_pred, u_ub_pred))) + \
              ...
              tf.reduce_mean(tf.square(tf.math.subtract(u_xx_lb_pred, u_xx_ub_pred)))
...
# define the physics-based residual f_u = u_t + u*u_x + u_xx + u_xxxx, want this to be 0
@tf.function
def f_model(x, t):
...
def predict(X_star):
    ...
    u_star, _, _ = u_x_model(u_model, X_star[:, 0:1], X_star[:, 1:2])
...
# train loop
parser = argparse.ArgumentParser(description="Train Kuramoto-Sivashinsky PINN")
parser.add_argument(
    "--optimizer",
